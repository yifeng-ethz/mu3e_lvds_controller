...
| B026 | D | Control reset release independent of data reset | 1 | Release `rsi_control_reset` only, hold `rsi_data_reset`, and read a dynamic counter aperture. | CSR identity and dynamic snapshot reads complete without wedging `waitrequest`; `coe_ctrl_pllrst` is owned by the control clock reset manager. | TBD |
